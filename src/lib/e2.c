...
unsigned char e2_read_byte(unsigned char addr) {
    do {                                                // wait until e2prom ready
        i2c_start();
        if (i2c_write_byte(E2PROM_ADDR << 1)) {
    ...
    i2c_write_byte(addr);
    i2c_start();
    i2c_write_byte((E2PROM_ADDR << 1) | 0x01);               // read operation
...
__bit e2_write_byte(unsigned char addr, unsigned char dat) {
    i2c_start();
    i2c_write_byte(E2PROM_ADDR << 1);
    i2c_write_byte(addr);
    __bit is_write_suc = i2c_write_byte(dat);
    i2c_stop();
    return is_write_suc;
}
...
unsigned char e2c_read_byte_256(unsigned int addr) {
    i2c_start();
    i2c_write_byte(E2PROM_ADDR << 1);                       // write operation
    i2c_write_byte(addr >> 8);
    i2c_write_byte(addr &= 0x00ff);
    i2c_start();
    i2c_write_byte((E2PROM_ADDR << 1) | 0x01);              // read operation
    unsigned char dat = i2c_read_NAK();
    i2c_stop();
